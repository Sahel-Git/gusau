...
// In a real app, logic would go to a controller
$ordersCount = 0;
$savedItemsCount = 0;
$totalSpent = 0;
$recentOrders = [];
...
<div class="flex flex-col gap-6 w-full max-w-7xl mx-auto py-4 px-4 sm:px-6 lg:px-8">
        ...
        <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
                ...
                <div class="mt-6 flex items-center gap-4">
                    <a href="<?php echo e(route('home')); ?>" class="rounded-full bg-white px-6 py-2.5 text-sm font-bold text-indigo-600 shadow-sm transition-all hover:bg-indigo-50 hover:scale-105">
    ...
    <div class="grid gap-6 sm:grid-cols-3 mt-2">
        <div class="flex items-center gap-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-zinc-200 dark:bg-zinc-900 border dark:border-zinc-800/50">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-zinc-900 dark:text-white"><?php echo e($ordersCount ?? 0); ?></p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-zinc-200 dark:bg-zinc-900 border dark:border-zinc-800/50">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-zinc-900 dark:text-white"><?php echo e($savedItemsCount ?? 0); ?></p>
            </div>
        </div>
        ...
        <div class="flex items-center gap-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-zinc-200 dark:bg-zinc-900 border dark:border-zinc-800/50">
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-zinc-900 dark:text-white">$<?php echo e(number_format($totalSpent ?? 0, 2)); ?></p>
            </div>
        </div>
    </div>
    ...
    <div class="mt-8 mb-4">
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(count($recentOrders ?? []) > 0): ?>
                ...
                <div class="bg-white dark:bg-zinc-900 rounded-xl p-5 border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col justify-between">
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <span class="font-bold text-zinc-900 dark:text-white">Order #<?php echo e($order->id); ?></span>
                            <span class="text-xs px-2.5 py-1 rounded-full <?php if($order->status == 'completed'): ?> bg-green-100 text-green-700 <?php elseif($order->status == 'pending'): ?> bg-yellow-100 text-yellow-700 <?php else: ?> bg-zinc-100 text-zinc-700 <?php endif; ?> font-semibold uppercase">
                                <?php echo e($order->status); ?>

                            </span>
                        </div>
                        <div class="text-sm text-zinc-500 dark:text-zinc-400 mb-4"><?php echo e($order->created_at->format('M d, Y H:i')); ?></div>
                    </div>
                    <div class="flex justify-between items-end border-t border-zinc-100 dark:border-zinc-800 pt-3">
                        ...
                        <span class="font-bold text-lg text-indigo-600 dark:text-indigo-400">$<?php echo e(number_format($order->total_amount, 2)); ?></span>
                    </div>
                </div>
...
<?php echo $__env->make('user.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\PROJECTS\gusau\resources\views/user/dashboard.blade.php ENDPATH**/ ?>