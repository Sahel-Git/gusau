...
@section('content')
        ...
        <div class="hero-container">
            ...
            <div class="hero-left">
                <div style="position: relative; z-index: 10;">
                    ...
                    <a href="{{ route('categories.index') }}" style="display: inline-block; text-decoration: none; background: white; color: #047857; text-transform: uppercase; font-weight: 800; padding: 1rem 2.5rem; border-radius: 9999px; border: none; font-size: 1.125rem; cursor: pointer; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.2); transition: transform 0.2s;">
                        Shop Now
                    </a>
                </div>
                <!-- Decorative Elements -->
            ...
            <div class="hero-right">
                ...
                <div style="flex: 1; display: flex; flex-direction: column; justify-content: space-around;">
                    @php $heroVendors = is_object($vendors ?? null) ? $vendors->take(4) : array_slice($vendors ?? [], 0, 4); @endphp
                    @if(!empty($heroVendors) && count($heroVendors))
                        @foreach($heroVendors as $vendor)
                        <a href="{{ route('store.show', data_get($vendor, 'slug', 'mock-store')) }}" class="hero-vendor-card">
                            <img src="{{ data_get($vendor, 'logo_path') ? asset('storage/'.data_get($vendor, 'logo_path')) : 'https://via.placeholder.com/60' }}" style="width: 50px; height: 50px; border-radius: 50%; object-fit: cover;" onerror="this.outerHTML='<div style=\'width:50px;height:50px;background:#e5e7eb;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:24px;\'>🏪</div>'">
                            <div style="flex: 1; overflow: hidden;">
                                <h4 style="margin: 0; font-size: 0.95rem; font-weight: 700; color: #1f2937; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{{ data_get($vendor, 'name', 'Store Name') }}</h4>
                                <div style="font-size: 0.8rem; color: #6b7280; margin-top: 2px;">{{ data_get($vendor, 'category.name', 'Top Seller') }}</div>
                                <div style="display: flex; align-items: center; gap: 4px; margin-top: 4px;">
                                    <span style="color: #fbbf24; font-size: 0.875rem;">★</span>
                                    <span style="font-size: 0.8rem; font-weight: 600; color: #374151;">{{ data_get($vendor, 'rating', '4.9') }}</span>
                                </div>
                            </div>
                        </a>
                        @endforeach
    ...
    <div id="trending-section">
        @include('user.partials.section-title', ['title' => 'Trending Near You', 'action' => 'See All'])
        <div class="u-horizontal-scroll">
            @if(!empty($trendingProducts) && count($trendingProducts))
    ...
    <div id="fast-delivery-section">
        @include('user.partials.section-title', ['title' => 'Fast Delivery', 'subtitle' => 'Get it today. Delivered under 30 mins.', 'action' => 'Explore'])
        <div class="u-horizontal-scroll">
            @if(!empty($fastDeliveryProducts) && count($fastDeliveryProducts))
                @foreach ($fastDeliveryProducts ?? [] as $product)
                @include('user.partials.product-card', ['product' => $product])
                @endforeach
            @else
                <p style="padding:10px;">No items available</p>
    ...
    <div id="trusted-vendors-section">
        @include('user.partials.section-title', ['title' => 'Trusted Vendors', 'action' => 'View More'])
        <div class="u-horizontal-scroll">
            @forelse ($vendors ?? [] as $vendor)
                @include('user.partials.vendor-card', ['vendor' => $vendor])
            @empty
                @for($i=0; $i<2; $i++)
                    @include('user.partials.vendor-skeleton')
                @endfor
            @endforelse
        </div>
    </div>
    ...
    <div id="flash-deals-section">
        @include('user.partials.section-title', ['title' => 'Flash Deals ⚡', 'action' => 'See All'])
        ...
        <div class="u-horizontal-scroll">
            @if(!empty($flashDeals) && count($flashDeals))
    ...
    <div id="recently-viewed-section">
        @include('user.partials.section-title', ['title' => 'Recently Viewed'])
        <div class="u-horizontal-scroll">
    ...
    <div id="buy-again-section">
        @include('user.partials.section-title', ['title' => 'Buy Again', 'subtitle' => 'Based on your recent activity'])
        <div class="u-horizontal-scroll">
            @if(!empty($recentlyViewed) && count($recentlyViewed))
                @foreach ($recentlyViewed ?? [] as $product)
                @include('user.partials.product-card', ['product' => $product])
    ...
    <div id="recommended-section">
        @include('user.partials.section-title', ['title' => 'Recommended For You'])
        <div class="u-horizontal-scroll">
            @if(!empty($recommendedProducts) && count($recommendedProducts))
