...
<header class="marketplace-header">
    <div class="mh-top">
        <!-- LEFT: Logo -->
        <a href="{{ route('welcome') }}" class="mh-logo">
        ...
        <div class="mh-icons">
            <div class="mh-login-text" style="display: flex; gap: 8px;">
                @guest
                    <a href="{{ route('login') }}" style="color: #4b5563; text-decoration: none;">Login</a>
                    <span style="color: #d1d5db;">/</span>
                    <a href="{{ route('register') }}" style="color: #4b5563; text-decoration: none;">Register</a>
                @endguest
                @auth
                    <a href="{{ route('user.dashboard') }}" style="color: #10b981; text-decoration: none;">My Account</a>
                @endauth
            </div>
            ...
            @endphp
            <a href="{{ route('cart.index') }}" style="color: #4b5563; position: relative;">
                <svg style="width: 26px; height: 26px;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                <!-- Badge -->
                @if($cartCount > 0)
                <span style="position: absolute; top: -6px; right: -8px; background: #ef4444; color: white; border-radius: 50%; width: 18px; height: 18px; font-size: 0.7rem; font-weight: bold; display: flex; align-items: center; justify-content: center;">{{ $cartCount }}</span>
                @endif
            </a>
        </div>
    </div>
    ...
    <div class="mh-nav">
        <div class="mh-nav-inner">
            <a href="{{ route('home') }}" class="mh-nav-item {{ request()->routeIs('home') ? 'active' : '' }}">Home</a>
            <a href="{{ route('categories.index') }}" class="mh-nav-item {{ request()->routeIs('categories.*') ? 'active' : '' }}">Categories</a>
            <a href="{{ route('services.index') }}" class="mh-nav-item">Services</a>
            <a href="{{ route('deals.index') }}" class="mh-nav-item">Top Deals</a>
            <a href="{{ route('contact.index') }}" class="mh-nav-item">Contact Us</a>
        </div>
    </div>
</header>
